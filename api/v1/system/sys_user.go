...
import (
	...
	systemReq "github.com/yaoyaochil/bodo-admin-server/server/model/system/request"
	systemRes "github.com/yaoyaochil/bodo-admin-server/server/model/system/response"
...
func (b *BaseApi) Login(c *gin.Context) {
	...
	if store.Verify(l.CaptchaId, l.Captcha, true) {
		...
		LastLoginTime := utils.GetCurrentTime()

		// 验证码正确
		u := &system.SysUser{Username: l.Username, Password: l.Password, LastLoginIp: LastLoginIp, LastLoginTime: LastLoginTime}
...
func (b *BaseApi) GetUserList(c *gin.Context) {
	var pageInfo request.SystemUserRequest
...
// @accept    application/json
// @Produce   application/json
// @Param     data  body      bodo_request.GetById                true  "用户ID"
// @Success   200   {object}  response.Response{msg=string}  "删除用户"
// @Router    /user/deleteUser [delete]
func (b *BaseApi) DeleteUser(c *gin.Context) {
	var reqId bodo_request.GetById
...
// @Router    /user/getUserTotal [get]
func (b *BaseApi) GetUserTotal(c *gin.Context) {
	total, err := userService.GetUserTotal()
	if err != nil {
		global.BODO_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
		return
	}
	response.OkWithDetailed(gin.H{"total": total}, "获取成功", c)
}
